...
\begin{document}
	...
	% 2. rho
	% 3. nominal coverage should be 95%: boxplot of posterior means, including coverage (one setting is enough for the moment)
	...
	Figure \ref{fig:E1_no_burn_gamma_tp} shows the traceplot of the parameter $\gamma$ among the first $10000$ iterations. We observe that the Markov chain quickly migrates from the low density region where it is initialized to the high density region where it mixes well. The acceptance rate of the latent spaces proposed in the Metropolis-Hastings step is $0.18$.
	...
	\begin{figure}
		...
		\begin{subfigure}[b]{0.41\textwidth}
			...
			\label{fig:E1_no_burn_gamma_hist}
		\end{subfigure}
		\hfill
		\begin{subfigure}[b]{0.41\textwidth}
			\centering
			\includegraphics[width=\textwidth]{E1_no_burn_gamma_acf}
			\caption{$R_0$}
			\label{fig:\textbf{traceplot_R0}}
		\end{subfigure}
