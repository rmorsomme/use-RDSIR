...
\begin{document}
	...
	\title{%Efficient Likelihood-Based Inference for Fitting Stochastic Epidemic Models to Incidence Data via Data Augmentation
		\large Uniformly Ergodic Data-Augmented MCMC for Fitting the General Stochastic Epidemic Model to Incidence Data}
		
%	\author{
%	Rapha\"{e}l Morsomme$^{1}$ \ and Jason Xu$^{1}$ \\ 
%	\small %\texttt{raphael.morsomme@duke.edu} \qquad
%	\texttt{jason.q.xu@duke.edu}\\
...
%	}
	...
	\end{abstract}
	\keywords{Stochastic epidemic model; incidence data; exact Bayesian inference; likelihood-based inference}
	...
	%	$$
	%	P(\mathbf{X}_t = \mathbf{x}, \mathbf{X}_{t+dt} = \mathbf{x}') =
	%	\begin{cases}
	%		\beta I(t) dt + o(dt), & \mathbf{x} \text{ and } \mathbf{x}' \text{ only differ at position } j \text{ with } x_j = S \text{ and } x_j' = I, \\
	%		\gamma dt + o(dt), & \mathbf{x} \text{ and } \mathbf{x}' \text{ only differ at position } j \text{ with } x_j = I \text{ and } x_j' = R, \\
	%		o(dt), & \text{otherwise}
	%	\end{cases}
	%	$$
	%	where $dt>0$ is small.
	...
	%	\begin{equation}
%	    \label{eq:lik2}
%	    L(\mathbf{X}|\tilde{\theta}) = \beta^{n_I + n_R} R_0^{-n_R} S(0)^{n_R} \prod_{j\in\mathcal{I}} I(\tau^I_j) \exp\left\lbrace - \beta \int_{0}^{t_{end}} I(t)S(t) - \frac{\beta S(0)}{R_0} \int_{0}^{t_{end}} I(t) dt \right\rbrace,
%	\end{equation}
	...
	\begin{equation}
		...
		%\mathbf{1}\{\mathbf{Y} \overset{\Delta}{=} \mathbf{x}\}
	\end{equation}
	...
	More recently, Bu et al.\ \cite{bu2020likelihood} propose a sampler for epidemics over networks when the infection times are assumed to be known, and Fintzi et al.\ \cite{Fintzi.2017} proposed an MCMC algorithm to conduct inference with discretely observed prevalence counts, with latent data consisting of the infection and recovery times of each individual. The algorithm explores this latent space by updating the event times of an individual per iteration of the algorithm. %These event times are sampled from a distribution that approximate the exact dynamics of the epidemic process.
	...
	Instead, we generate the latent data conditionally on $\mathbf{Y}$ from a surrogate process, and accept or reject them in a Metropolis-Hastings (MH) step. Given some current $\theta$ and $\mathbf{z}$, the MH step proceeds by simulating a candidate $\mathbf{z}^\star$ from a surrogate process $Q(.|\theta)$ with density $q(.|\theta)$, which we present in Section \eqref{sec:pds}. Then $\mathbf{z}^\star$ is accepted with probability
	\begin{equation}
		\label{eq:alpha}
	...
	\end{equation}
	and otherwise the current $\mathbf{z}$ is retained \cite{Tierney.1994}. Note that it is not necessary to compute $\delta_{\mathbf{Y}}(\mathbf{z})$ in the MH acceptance ratio \eqref{eq:alpha} since $\mathbf{z}^\star$ is compatible with $\mathbf{Y}$ by construction. Algorithm \ref{alg:DA-MCMC} provides the details of the DA-MCMC algorithm.
	...
	\begin{figure}
		...
		Vertical dotted lines indicate infection (red) and removal (blue) times.}
		\label{fig:mu}
	\end{figure}
	...
	\label{eq:q}
		q(\mathbf{z}|\theta) 
		& = && \prod_{k=1}^K \prod_{j\in\mathcal{I}_k} \textsf{TruncExp}(z^I_j; \mu_{k}, t_{k-1}, t_{k}) \\
		& && \times \prod_{i=1}^{n} \left( 1 - p_i \right)^{\mathbf{1}(z^R_i = \infty)} \left( p_i \textsf{TruncExp}(z^R_i; \gamma, z^I_i, t_{end}) \right)^{\mathbf{1}(z^R_i \le t_{end})}
	\end{alignat}
	where %$\mu_k = \beta I(t_{k-1})$ and
	$$\textsf{TruncExp}(x; \mu, l, u) = \dfrac{\mu \exp\{-\mu x\}}{\exp\{-\mu l\} - \exp\{-\mu u\}}, \quad x \in (l, u)$$
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\FOR {$j = 1, \dots, I(0)$}
    			...
    			\STATE $z^R_j \sim (1 - p_j) \delta_\infty + p_j \textsf{TruncExp}(\gamma, 0, t_{end})$
			\ENDFOR
			...
			\FOR {$k = 1, \dots, K$}
    			...
    			\STATE $X_{\mathcal{I}_k} \sim \textsf{TruncExp}(\mu_k, t_{k-1}, t_k)$ i.i.d.\
    			
    			\FOR {$j \in \mathcal{I}_k$}
        			\STATE $z^I_j \leftarrow X_{(j)}$ (infection times)
        			\STATE $p_j \leftarrow 1 - \exp\{-\gamma(t_{end} - z^I_j)\}$
        			\STATE $z^R_j \sim (1 - p_j) \delta_\infty + p_j \textsf{TruncExp}(\gamma, z^I_j, t_{end})$ (removal times)
    			\ENDFOR
	...
	Third, if all event times are updated ($\rho = 1$), then the proposed and current latent data are independent conditionally on the current values of the parameters: $\mathbf{Z}^{(k-1)} \perp \mathbf{Z}^* | \theta^{(k)}$. Our proposal scheme can therefore be said to be semi-independent, a characteristic that is crucial for proving the uniform ergodicity of the resulting Markov chain in Section \eqref{sec:uni}. This contrasts with existing DA-MCMC algorithms which only update a small fraction of the latent data per iteration and generate consecutive configurations of the latent data that are mostly identical. 
...
%	$$
	...
	$m=\lfloor \rho^{-1} \rfloor$ and for some $\epsilon$ and $\nu$.
	...
	to a SSU DA-MCMC that is similar in spirit to those in \cite{Gibson.1998, ONeill.1999, Fintzi.2017}. We do not consider particle filters \cite{King.2015} nor diffusion approximations \cite{Fintzi.2020} since these methods are not suited to situations in which do not provide a noisy distribution modeling counts; see \cite{Ho.2018}.
	...
	The outbreak is believed to have originated in the Gu\'eck\'edou prefecture of Guinea at the end of November $2013$ \cite{Coltart.2017}. Weekly infection incidence counts are available for each prefecture for the $73$ weeks between the end of December $2013$ and May $2015$.
	...
	the stochastic SIR model for which our algorithm is designed. However, one can argue that it is a simplistic representation of the spread of disease, relying on assumptions such as perfect reporting, exponentially-distributed infectious periods, homogeneously mixing population and a constant infection rate. Our data-augmentation framework and the idea of a faithful mechanistic proposal can be extended to models of increasing realism where these assumptions are relaxed. It will be fruitful to consider extensions to epidemic models accounting for under-reporting \cite{Fintzi.2017,morozova2021one}, non-Markovian dynamics \cite{Streftaris.2002}, non-homogeneous mixing \cite{Severo.1969,martinez2021statistical} as well as time-varying infection rates \cite{Kypraios.2018} in future work. We invite readers to consider applications of these ideas in other stochastic process models with complex latent spaces that may similarly be made navigable through designing tractable, mechanistic proposal processes.
	...
	\begin{proof}
		...
		First, we can obtain a closed form minorization of $\pi(\beta_2| \mathbf{z}_1)$ using Proposition \ref{pro:ga1} as follows,
		\begin{align*}
		...
		$\int_{0}^{t_{end}}S(t)I(t)dt$, is bounded between $0$ and $n (n_I+I_0)t_{end}$. Similarly, by Proposition \ref{pro:ga2}, we can minorize $\pi(\gamma_2| \mathbf{z}_1)$ as follows,
		\begin{align*}
		...
		\begin{align*}
		    ...
		    & = \dfrac{\prod_{k=1}^K \prod_{j\in \mathcal{I}_k} \textsf{TruncExp}(z^I_j; \mu_k, t_{k-1}, t_k) \prod_{i=1}^n (1-p_i)^{\mathbf{1}\{z^R_i=\infty\}} \left(p_i \textsf{TruncExp}(z^R_i;\gamma, z^I_i, t_{end})\right)^{\mathbf{1}\{z^R_i \le t_{end}\}}}
        	{\beta^{n_I} \prod_{l\in \mathcal{I}} I(z^I_l) \gamma^{n_R} \exp\{-\beta \int_0^{t_{end}} S(t)I(t)dt -\gamma \int_0^{t_{end}} I(t)dt\}} \\
        	& = \dfrac{\prod_{k=1}^K \prod_{j\in \mathcal{I}_k} \textsf{TruncExp}(z^I_j; \mu_k, t_{k-1}, t_k)}
        ...
        \begin{align*}
        	& \dfrac{\prod_{i=1}^n (1-p_i)^{\mathbf{1}\{z^R_i=\infty\}} (p_i \textsf{TruncExp}(z^R_i;\gamma, z^I_i, t_{end}))^{\mathbf{1}\{z^R_i \le t_{end}\}}}{\gamma^{n_R} \exp\{- \gamma \int_0^{t_end} I(t)dt\}} \\
...
%	\begin{proof}
%		Remember that $\textsf{TruncExp}(u; \mu, 0, u) = \frac{\mu \exp\{-u\mu\}}{1-\exp\{-u\mu\}}$ is the density of an exponential distribution bounded above by $u$.
%		\begin{align*}
%			\frac{d}{d\mu}\textsf{TruncExp}(u; \mu, 0, u)
...
%		This implies that $g(\mu) \le 0$ for $\mu>0$. We therefore have $\frac{d}{d\mu}\textsf{TruncExp}(u; \mu, 0, u) \le 0$ and the result of the proposition follows.
%	\end{proof}
